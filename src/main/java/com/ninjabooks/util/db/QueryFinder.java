...
@Component
@Transactional
public class QueryFinder implements SpecifiedElementFinder
{
    ...
    public <T, E> T findSpecifiedElementInDB(E parameter, Enum columnName) {
        ...
        String hsqlQuery =
            "SELECT element FROM " + classType.getSimpleName() + " element WHERE " + columnName + "=:parameter";
        Query<T> query = currentSession.createQuery(hsqlQuery);
        query.setParameter("parameter", parameter);

        return (T) query.uniqueResultOptional();
    }
}
